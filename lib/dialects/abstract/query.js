...
var groupJoinData = function(rows, includeOptions, options) {
  ...
  var
    ...
    //process the rows in chunks with setImmediate in between to break up the really long stretch of synchronous
    //code that locks up the node event loop
  var chunkSize = 500;
  var chunks = Utils._.chunk(rows, chunkSize);

  const processChunk = (chunk, chunkIndex) => {
    for (let rowsC = 0; rowsC < chunk.length; rowsC++) {
      if (chunkIndex === 0) {
        rowsI = rowsC;
      } else {
        rowsI = (chunkIndex * chunkSize) + rowsC;
      }
      row = chunk[rowsC];
    ...
    if (chunkIndex === chunks.length - 1) {
      ...
      chunkIndex++;
      return setImmediate(() => {
        return processChunk(chunks[chunkIndex], chunkIndex);
      })
    }
  }

    return processChunk(chunks[0], 0);
  })
};
  ...
  return new Promise((resolve) => {
    ...
    if (this.options.raw) {
      ...
      return groupJoinData(results, {
        ...
        return self.model.bulkBuild(results, {
          isNewRecord: false,
          include: self.options.include,
          includeNames: self.options.includeNames,
          includeMap: self.options.includeMap,
          includeValidated: true,
          attributes: self.options.originalAttributes || self.options.attributes,
          raw: true,
          //request as a promise then we want to break up the event loop here by injecting some setImmediates
          //This will only breakup between top level objects. Its a larger task to do between all sub objects
          returnPromise: true
        }).then(result => {
