...
Model.prototype.sync = function(options) {
  ...
  return Promise.try(function () {
    ...
    return self.QueryInterface.createTable(self.getTableName(), attributes, options, self);
  }).then(function () {
    ...
    return Promise.map(indexes, function (index) {
      return self.QueryInterface.addIndex(self.getTableName(), _.assign({logging: options.logging}, index), self.tableName);
    });
  }).then(function () {
    if (options.hooks) {
...
Model.prototype.schema = function(schema, options) { // testhint options:none
  ...
  if (!!options) {
    if (typeof options === 'string') {
      self.$schemaDelimiter = options;
    } else {
      if (!!options.schemaDelimiter) {
        self.$schemaDelimiter = options.schemaDelimiter;
      }
    }
  }
...
Model.prototype.getTableName = function(options) { // testhint options:none
  if(!options) {
    ...
    return this.QueryGenerator.addSchema({
      ...
      $schemaDelimiter: options.$schemaDelimiter || this.$schemaDelimiter
    });
  }
};
...
Model.prototype.upsert = function (values, options) {
  ...
  return instance.hookValidate(options).bind(this).then(function () {
    ...
    return this.QueryInterface.upsert(this.getTableName(), insertValues, updateValues, this, options);
  });
};
...
Model.prototype.bulkCreate = function(records, options) {
  ...
  return Promise.try(function() {
    ...
    if (options.individualHooks) {
      ...
      options.model = self;
      return self.QueryInterface.bulkInsert(self.getTableName(), records, options, attributes).then(function (results) {
